classdef ads1015 < handle & matlab.mixin.CustomDisplay
    ...
    %
    % Originally created murat_belge
    % https://forums.raspberrypi.com/viewtopic.php?t=84491
    %
    ...
    % you used the ADDR pin to change the I2C address of the ads1015 from
    % the default '0x48' to something else.
    %
    % readVoltage(adc, AINp) reads the single-ended voltage measurement
    % from AINp input port. 
    %
    % [V,R] = readVoltage(adc, AINp) reads the single-ended voltage measurement
    % from AINp input port as well as the raw conversion register value. 
    %
    ...
    %https://www.ti.com/lit/ds/symlink/ads1015.pdf?ts=1638550150965&ref_url=https%253A%252F%252Fwww.google.com%252F
    % NOTE: Do not apply voltages excedding VDD+0.3V to any input pin.
    
    % Copyright 2014 The MathWorks, Inc.
    
    % This file was modified to work with ADS1015 inside Adafruits Joy
    % Bonnet. The Retrogame code was uses as a templete.
    % https://github.com/adafruit/Adafruit-Retrogame/blob/master/joyBonnetAsMouse.py
    ...
    properties (Constant, Access = private)
        ...
        % Full scale for ads1015 is 4.096 volts
        FS_VOLTAGE = 4.096
        
        % 12-bit ADC result needs to be scaled by this value, see Data
        % Format in datasheet
        ADC_SCALAR = 2^11 - 1
    end
    ...
    methods
        function obj = ads1015(raspiObj, bus, address)
            ...
            obj.VoltageScale = 4.096; % This will let you read up to VDD. Joybonnet is 3.3V
            
            % Initialize config register value
            % COMP_LAT, _POL, and _MODE are never used, so they are 0 from
            % here on
            obj.ConfigReg = 0;
        ...
        function [voltage, data4bitshiftnumeric] = readVoltage(obj, AINp, AINn)
            ...
            data4bitshiftnumeric = bitshift(double(swapbytes(data)),-4);
            % Added bitshift due to 1015 being 12 bit not 16 bit like 1115
            voltage = bitshift(double(swapbytes(data)),-4) * ...
                (obj.VoltageScale) / obj.ADC_SCALAR;
        end
    end
    ...
    methods (Access = protected)
        function displayScalarObject(obj)
            ...
            fprintf('          OperatingMode: %-15s (''single-shot'' or ''continuous'')\n', ...
                obj.OperatingMode);
            fprintf('       SamplesPerSecond: %-15d (128, 250, 490, 920, 1600, 2400, or 3300)\n', ...
...
%                     DRbits = 7;
            ...
            if AINn == -1
                ...
                if (AINp == 3) && (AINn == 2)
                    MUXbits = 0;
                elseif (AINp == 3) && (AINn == 0)
                    MUXbits = 1;
                elseif (AINp == 2) && (AINn == 0)
                    MUXbits = 2;
                elseif (AINp == 1) && (AINn == 0)
                    MUXbits = 3;
                else
